...
exports.addToCart = async (req, res) => {
  try {
    ...
    if (existing) {
      if (product.stock < existing.quantity + quantity) return res.status(400).json({ error: 'Not enough stock available.' });
...
exports.removeFromCart = async (req, res) => {
  try {
    const { error } = Joi.object({ productId: Joi.string().required() }).validate(req.body);
    if (error) return res.status(400).json({ error: error.details[0].message });
